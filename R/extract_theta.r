...
#' @param obstype Character string with the name of the probability distribution of the observations.  This controls the back-transformation of the process parameters.  Possible values for \code{obstype} are 'normal', 'poisson', or 'binomial', or 'coalescent'.
...
extract_theta <- function(mfit, obstype="normal",  alpha=0.05){
  ...
  if ( !(obstype %in% c("normal", "poisson", "binomial", "coalescent") ) ) stop("Argument 'obstype' must be 'normal', 'poisson', 'binomial', or 'coalescent'.")
  ...
  if (obstype=="poisson" | obstype=="coalescent"){
