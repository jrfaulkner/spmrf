...
The following document provides `R` code for fitting shrinkage prior Markov random field models using the `spmrf` package applied to a data example on coal mine disasters which is presented in Faulkner and Minin (2018). See the [Introduction to the spmrf package](introduction_to_spmrf.html) for instructions on installation and more details about the package.
